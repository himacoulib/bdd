package ml.satgrie.pl.ue.model;
// Generated 31 mars 2021 � 17:28:15 by Hibernate Tools 5.2.12.Final

import javax.persistence.Column;
import javax.persistence.Embeddable;
import javax.persistence.GeneratedValue;
import javax.persistence.GenerationType;

/**
 * CercleId generated by hbm2java
 */
@Embeddable
public class CercleId implements java.io.Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private int paysId;
	private int regionId;
	private int cerlceId;

	public CercleId() {
	}

	public CercleId(int paysId, int regionId, int cerlceId) {
		this.paysId = paysId;
		this.regionId = regionId;
		this.cerlceId = cerlceId;
	}

	@Column(name = "PaysID", nullable = false)
	public int getPaysId() {
		return this.paysId;
	}

	public void setPaysId(int paysId) {
		this.paysId = paysId;
	}

	@Column(name = "RegionID", nullable = false)
	public int getRegionId() {
		return this.regionId;
	}

	public void setRegionId(int regionId) {
		this.regionId = regionId;
	}

	@Column(name = "CerlceID", nullable = false)
	public int getCerlceId() {
		return this.cerlceId;
	}

	public void setCerlceId(int cerlceId) {
		this.cerlceId = cerlceId;
	}

	@Override
	public boolean equals(Object other) {
		if ((this == other))
			return true;
		if ((other == null))
			return false;
		if (!(other instanceof CercleId))
			return false;
		CercleId castOther = (CercleId) other;

		return (this.getPaysId() == castOther.getPaysId()) && (this.getRegionId() == castOther.getRegionId())
				&& (this.getCerlceId() == castOther.getCerlceId());
	}

	@Override
	public int hashCode() {
		int result = 17;

		result = 37 * result + this.getPaysId();
		result = 37 * result + this.getRegionId();
		result = 37 * result + this.getCerlceId();
		return result;
	}

}
